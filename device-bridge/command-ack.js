...
function verifiedChatReturnFoundationNotReadyError() {
  return new DeviceBridgeProtocolError(
    503,
...
async function assertVerifiedChatReturnFoundationReady(client, assertFoundationReady) {
  try {
    await assertFoundationReady(client);
  } catch {
...
export async function processCommandAckTransaction(pool, auth, ack, now = new Date(), {
  ...
  try {
    ...
    if (expired && TINDER_VERIFIED_CHAT_RETURN_COMMANDS.has(command.command_type)
        && ack.status === "SUCCEEDED") {
    ...
    if (command.command_type === "RESUME_OFFICIAL_TINDER_APP"
        && ack.status === "SUCCEEDED") {
      try {
        const inspection = await inspectVerifiedChatReturnSchema(client);
        returnFoundationReady = inspection?.state === TINDER_VERIFIED_CHAT_RETURN_FOUNDATION_STATE.CANONICAL;
      } catch {
