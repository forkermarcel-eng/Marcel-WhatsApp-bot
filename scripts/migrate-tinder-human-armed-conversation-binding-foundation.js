...
import {
  ...
  HUMAN_ARMED_CONVERSATION_BINDING_MIGRATION_DIAGNOSTIC_REASONS,
  HUMAN_ARMED_CONVERSATION_BINDING_MIGRATION_DIAGNOSTIC_STAGES,
...
const REASONS = new Set(HUMAN_ARMED_CONVERSATION_BINDING_MIGRATION_DIAGNOSTIC_REASONS);
...
function logDiagnostic(logger, diagnostic) {
  ...
  logger.error(
    `Human-armed conversation binding migration diagnostic: stage=${value.stage} code=${value.code} `
      + `transaction=${value.transaction} rollback=${value.rollback} ddl_started=${value.ddl_started}`
      + (value.reason ? ` reason=${value.reason}` : "")
  );
}

export async function runHumanArmedConversationBindingFoundationMigrationCli({
  ...
  migrate = migrateHumanArmedConversationBindingFoundation,
  getFailureDiagnostic = getHumanArmedConversationBindingFoundationMigrationFailureDiagnostic,
  logger = console
} = {}) {
  if (!argv.includes("--apply")) {
    logger.error("Refusing human-armed conversation binding migration without --apply.");
  ...
  if (!environment.DATABASE_URL) {
    logger.error("Human-armed conversation binding migration requires DATABASE_URL.");
    logDiagnostic(logger, {
  ...
  if (diagnostic) {
    logger.error("Human-armed conversation binding migration failed.");
    logDiagnostic(logger, diagnostic);
    return false;
  }
  logger.log(result.migrated
    ? "Human-armed conversation binding migration completed."
    : "Human-armed conversation binding foundation already canonical.");
  logger.log(
    `Human-armed conversation binding migration diagnostic: stage=COMMIT_CONFIRMED `
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  const success = await runHumanArmedConversationBindingFoundationMigrationCli();
