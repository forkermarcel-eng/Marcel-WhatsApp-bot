...
import {
  ...
  TINDER_LOCAL_CONVERSATION_ATTESTATION_MIGRATION_DIAGNOSTIC_REASONS,
  TINDER_LOCAL_CONVERSATION_ATTESTATION_MIGRATION_DIAGNOSTIC_STAGES
...
const STAGES = new Set([
  "CLI_ARGUMENT_VALIDATION", "ENVIRONMENT_VALIDATION", "COMMIT_CONFIRMED",
  ...TINDER_LOCAL_CONVERSATION_ATTESTATION_MIGRATION_DIAGNOSTIC_STAGES
]);
const CODES = new Set([
  "APPLY_REQUIRED", "DATABASE_URL_REQUIRED", "DATABASE_CONNECTION_FAILED",
  "MIGRATION_SOURCE_INVALID", "MIGRATION_APPLIED", "ALREADY_CANONICAL",
  "ADVISORY_LOCK_UNAVAILABLE", "LOCK_TIMEOUT", "COMMIT_OUTCOME_UNRESOLVED",
  "CLEANUP_FAILED", "DATABASE_OPERATION_FAILED"
]);
const REASONS = new Set(TINDER_LOCAL_CONVERSATION_ATTESTATION_MIGRATION_DIAGNOSTIC_REASONS);
...
function logDiagnostic(logger, diagnostic) {
  ...
  logger.error(
    `Tinder local conversation attestation migration diagnostic: stage=${value.stage} code=${value.code} `
      + `transaction=${value.transaction} rollback=${value.rollback} ddl_started=${value.ddl_started}`
      + (value.reason ? ` reason=${value.reason}` : "")
  );
}

export async function runTinderLocalConversationAttestationMigrationCli({
  ...
  migrate = migrateTinderLocalConversationAttestation,
  getFailureDiagnostic = getTinderLocalConversationAttestationMigrationFailureDiagnostic,
  logger = console
} = {}) {
  if (!argv.includes("--apply")) {
    logger.error("Refusing Tinder local conversation attestation migration without --apply.");
  ...
  if (!environment.DATABASE_URL) {
    logger.error("Tinder local conversation attestation migration requires DATABASE_URL.");
    logDiagnostic(logger, {
  ...
  if (diagnostic) {
    logger.error("Tinder local conversation attestation migration failed.");
    logDiagnostic(logger, diagnostic);
    return false;
  }
  logger.log(result.migrated
    ? "Tinder local conversation attestation migration completed."
    : "Tinder local conversation attestation foundation already canonical.");
  logger.log(
    `Tinder local conversation attestation migration diagnostic: stage=COMMIT_CONFIRMED `
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  const success = await runTinderLocalConversationAttestationMigrationCli();
