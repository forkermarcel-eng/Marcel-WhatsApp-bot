...
import {
  ...
  migrateTinderManualSendCommand,
  T5_MANUAL_SEND_COMMAND_MIGRATION_DIAGNOSTIC_STAGES
} from "../device-bridge/tinder-manual-send-command-migration.js";

/* Explicit, unrouted T5 vocabulary DDL CLI. It refuses calls without --apply. */
...
function boundedDiagnostic(value, fallback = {}) {
  return {
    ...
    reason: typeof value?.reason === "string" && /^[A-Z][A-Z0-9_]{0,95}$/.test(value.reason)
...
function logDiagnostic(logger, diagnostic) {
  ...
  logger.error(
    `T5 signed send-command migration diagnostic: stage=${value.stage} code=${value.code} `
      + `transaction=${value.transaction} rollback=${value.rollback} ddl_started=${value.ddl_started}`
      + (value.reason ? ` reason=${value.reason}` : "")
  );
}

export async function runTinderManualSendCommandMigrationCli({
  ...
  migrate = migrateTinderManualSendCommand,
  logger = console
} = {}) {
  if (!argv.includes("--apply")) {
    logger.error("Refusing T5 signed send-command migration without --apply.");
  ...
  if (!environment.DATABASE_URL) {
    logger.error("T5 signed send-command migration requires DATABASE_URL.");
  ...
  try {
    ...
    diagnostic = getTinderManualSendCommandMigrationFailureDiagnostic(error) || (migrationStarted
  ...
  if (diagnostic) {
    logger.error("T5 signed send-command migration failed.");
    logDiagnostic(logger, diagnostic);
    return false;
  }
  logger.log(result.migrated
    ? "T5 signed send-command migration completed."
    : "T5 signed send-command vocabulary already canonical.");
  logger.log(
    `T5 signed send-command migration diagnostic: stage=COMMIT_CONFIRMED code=${result.migrated ? "MIGRATION_APPLIED" : "ALREADY_CANONICAL"} `
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  const success = await runTinderManualSendCommandMigrationCli();
