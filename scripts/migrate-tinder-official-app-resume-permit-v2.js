...
import {
  ...
  TINDER_OFFICIAL_APP_RESUME_PERMIT_V2_MIGRATION_DIAGNOSTIC_REASONS,
  TINDER_OFFICIAL_APP_RESUME_PERMIT_V2_MIGRATION_DIAGNOSTIC_STAGES
...
const REASONS = new Set(TINDER_OFFICIAL_APP_RESUME_PERMIT_V2_MIGRATION_DIAGNOSTIC_REASONS);
...
function logDiagnostic(logger, diagnostic) {
  ...
  logger.error(
    `Tinder official-app resume permit V2 migration diagnostic: stage=${value.stage} code=${value.code} `
      + `transaction=${value.transaction} rollback=${value.rollback} ddl_started=${value.ddl_started}`
      + (value.reason ? ` reason=${value.reason}` : "")
  );
}

export async function runTinderOfficialAppResumePermitV2MigrationCli({
  ...
  migrate = migrateTinderOfficialAppResumePermitV2,
  getFailureDiagnostic = getTinderOfficialAppResumePermitV2MigrationFailureDiagnostic,
  logger = console
} = {}) {
  if (!argv.includes("--apply")) {
    logger.error("Refusing Tinder official-app resume permit V2 migration without --apply.");
  ...
  if (!environment.DATABASE_URL) {
    logger.error("Tinder official-app resume permit V2 migration requires DATABASE_URL.");
    logDiagnostic(logger, {
  ...
  if (diagnostic) {
    logger.error("Tinder official-app resume permit V2 migration failed.");
    logDiagnostic(logger, diagnostic);
    return false;
  }
  logger.log(result.migrated
    ? "Tinder official-app resume permit V2 migration completed."
    : "Tinder official-app resume permit V2 foundation already canonical.");
  logger.log(
    `Tinder official-app resume permit V2 migration diagnostic: stage=COMMIT_CONFIRMED `
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  const success = await runTinderOfficialAppResumePermitV2MigrationCli();
