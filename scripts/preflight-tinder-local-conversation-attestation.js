...
import {
  validateTinderLocalConversationAttestationMigrationSource
...
/* Explicit read-only operation. It owns neither DDL nor a device action. */

const FOUNDATION_STATES = new Set([
  TINDER_LOCAL_CONVERSATION_ATTESTATION_FOUNDATION_STATE.UPGRADE_REQUIRED,
...
const REASONS = new Set([
  ...
  "LOCAL_CONVERSATION_ATTESTATION_SCHEMA_INCOMPATIBLE",
  "LOCAL_CONVERSATION_ATTESTATION_ACTIVE_V1_PERMIT",
  "LOCAL_CONVERSATION_ATTESTATION_PREREQUISITE_INSPECTION_FAILED",
  "LOCAL_CONVERSATION_ATTESTATION_V1_CONSTRAINT_INSPECTION_FAILED",
  "LOCAL_CONVERSATION_ATTESTATION_ACTIVE_V1_PERMIT_CHECK_FAILED",
...
const STAGES = new Set([
  ...
  "ACTIVE_V1_PERMIT_CHECK",
  "ATTESTATION_SCHEMA_INSPECTION",
  "RESULT_VALIDATION",
...
function reasonForPreflightError(error) {
  ...
  if (error?.code === "TINDER_LOCAL_CONVERSATION_ATTESTATION_ACTIVE_V1_PERMIT") {
    return { reason: "LOCAL_CONVERSATION_ATTESTATION_ACTIVE_V1_PERMIT", stage: "ACTIVE_V1_PERMIT_CHECK" };
  }
  if (error?.code === TINDER_LOCAL_CONVERSATION_ATTESTATION_PREFLIGHT_ERROR_CODE.PREREQUISITE_INSPECTION_FAILED) {
    return { reason: "LOCAL_CONVERSATION_ATTESTATION_PREREQUISITE_INSPECTION_FAILED", stage: "PREREQUISITE_INSPECTION" };
  }
  if (error?.code === TINDER_LOCAL_CONVERSATION_ATTESTATION_PREFLIGHT_ERROR_CODE.V1_CONSTRAINT_INSPECTION_FAILED) {
    return { reason: "LOCAL_CONVERSATION_ATTESTATION_V1_CONSTRAINT_INSPECTION_FAILED", stage: "V1_CONSTRAINT_INSPECTION" };
  }
  if (error?.code === TINDER_LOCAL_CONVERSATION_ATTESTATION_PREFLIGHT_ERROR_CODE.ACTIVE_V1_PERMIT_CHECK_FAILED) {
    return { reason: "LOCAL_CONVERSATION_ATTESTATION_ACTIVE_V1_PERMIT_CHECK_FAILED", stage: "ACTIVE_V1_PERMIT_CHECK" };
  }
  if (error?.message === "Tinder local conversation attestation schema is incompatible.") {
    return { reason: "LOCAL_CONVERSATION_ATTESTATION_SCHEMA_INCOMPATIBLE", stage: "ATTESTATION_SCHEMA_INSPECTION" };
  }
...
function logResult(logger, result) {
  logger.log(
    `Tinder local conversation attestation read-only preflight: status=${result.ok ? "PASS" : "FAIL"} `
...
export async function runTinderLocalConversationAttestationPreflightCli({
  ...
  validateMigrationSource = validateTinderLocalConversationAttestationMigrationSource,
  ...
  try {
    ...
    const result = boundedResult({ ok: false, reason: "MIGRATION_SOURCE_INVALID", stage: "SOURCE_VALIDATION" });
    logResult(logger, result);
    return result;
  }
  if (!environment.DATABASE_URL) {
    const result = boundedResult({ ok: false, reason: "DATABASE_URL_REQUIRED", stage: "ENVIRONMENT_VALIDATION" });
    logResult(logger, result);
    return result;
  }
  ...
  try {
    ...
    if (!FOUNDATION_STATES.has(foundationState)
        || checked?.mutate !== (foundationState === TINDER_LOCAL_CONVERSATION_ATTESTATION_FOUNDATION_STATE.UPGRADE_REQUIRED)) {
      result = boundedResult({
        ok: false, reason: "PREFLIGHT_FAILED",
        transaction: "READ_ONLY_REPEATABLE_READ", rollback: "COMPLETED", stage: "RESULT_VALIDATION"
      });
    } else {
      result = boundedResult({
        ok: true,
