...
import {
  validateTinderUnboundInboxConversationSweepTriggerRepairMigrationSource
...
/* Explicit V8 immutable-trigger repair preflight.  It owns neither DDL nor a device command. */
...
const REASONS = new Set([
  ...
  "READ_ONLY_TRANSACTION_FAILED", "UNBOUND_INBOX_CONVERSATION_SWEEP_TRIGGER_REPAIR_SCHEMA_INCOMPATIBLE",
...
function reasonForPreflightError(error) {
  ...
  if (error?.message === "Tinder unbound Inbox conversation sweep trigger repair schema is incompatible.") {
    return { reason: "UNBOUND_INBOX_CONVERSATION_SWEEP_TRIGGER_REPAIR_SCHEMA_INCOMPATIBLE", stage: "SCHEMA_INSPECTION" };
  }
  return { reason: "PREFLIGHT_FAILED", stage: "VALIDATION_UNCLASSIFIED" };
}

function logResult(logger, result) {
  logger.log(`Tinder unbound Inbox conversation sweep trigger repair read-only preflight: status=${result.ok ? "PASS" : "FAIL"} reason=${result.reason} foundation_state=${result.foundation_state} migration_required=${result.migration_required} transaction=${result.transaction} rollback=${result.rollback} stage=${result.stage}`);
}

export async function runTinderUnboundInboxConversationSweepTriggerRepairPreflightCli({
  ...
  readMigrationSource = () => readFileSync(new URL("../migrations/20260913_tinder_unbound_inbox_conversation_sweep_trigger_repair.sql", import.meta.url), "utf8"),
  validateMigrationSource = validateTinderUnboundInboxConversationSweepTriggerRepairMigrationSource,
  ...
  try {
    ...
    result = !FOUNDATION_STATES.has(foundationState) || checked?.mutate !== (foundationState === TINDER_UNBOUND_INBOX_CONVERSATION_SWEEP_FOUNDATION_STATE.TRIGGER_REPAIR_REQUIRED)
