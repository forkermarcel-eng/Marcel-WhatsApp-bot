...
import {
  runTinderAutomationControlFoundationMigrationCli
} from "../scripts/migrate-tinder-automation-control-foundation.js";
...
test("the explicit T7 CLI refuses to connect or migrate without --apply", async () => {
  const output = logger();
  let poolCreated = false;
  const result = await runTinderAutomationControlFoundationMigrationCli({
    argv: [],
...
test("the explicit T7 CLI requires a database setting before creating a pool", async () => {
  const output = logger();
  let poolCreated = false;
  const result = await runTinderAutomationControlFoundationMigrationCli({
    argv: ["--apply"],
...
test("the explicit T7 CLI emits only bounded success and closes its pool", async () => {
  const output = logger();
  let poolEnded = false;
...
test("the explicit T7 CLI fail-closes runner errors without exposing low-level detail", async () => {
  const output = logger();
  let poolEnded = false;
  const pool = { async end() { poolEnded = true; } };
